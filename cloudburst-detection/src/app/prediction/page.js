        ...
        <div className="max-w-7xl mx-auto px-4">
          <div className="flex items-center justify-center min-h-[400px]">
            <div className="text-center">
              ...
              <p className="text-gray-600 text-lg">Generating predictions...</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 py-8">
        <div className="max-w-7xl mx-auto px-4">
          <div className="bg-red-50 border border-red-200 rounded-lg p-8 text-center">
            ...
            <h2 className="text-2xl font-bold text-red-900 mb-2">Prediction Error</h2>
      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <h1 className="text-4xl font-bold text-gray-900">Cloudburst Predictions</h1>
              <p className="text-gray-600 mt-1">AI-powered weather predictions using XGBoost model</p>
            </div>
          </div>
          
          {predictionDate && (
            <div className="bg-white rounded-lg shadow-md p-4 inline-flex items-center gap-3">
              <Calendar className="h-5 w-5 text-blue-600" />
              <div>
                <span className="text-sm text-gray-600">Predictions for:</span>
                <p className="font-semibold text-gray-900">{formatDate(predictionDate)}</p>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          ...
          <div className="bg-white rounded-lg shadow-md p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-gray-600 text-sm">Medium Risk Areas</p>
                <p className="text-3xl font-bold text-yellow-600">
                  {predictions.filter(p => p.prediction >= 0.4 && p.prediction < 0.7).length}
                </p>
              </div>
              <CloudRain className="h-12 w-12 text-yellow-400" />
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg shadow-md p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-gray-600 text-sm">Low Risk Areas</p>
                <p className="text-3xl font-bold text-green-600">
                  {predictions.filter(p => p.prediction < 0.4).length}
                </p>
              </div>
              <CloudRain className="h-12 w-12 text-green-400" />
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {predictions.map((pred) => {
            ...
            return (
              <div 
                key={pred.id}
                className={`bg-white rounded-lg shadow-lg p-6 border-l-4 ${risk.borderColor} hover:shadow-xl transition-shadow`}
              >
                {/* Header */}
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-3">
                    <div className={`p-2 rounded-lg ${risk.bgColor}`}>
                      <CloudRain className={`h-6 w-6 text-${risk.color}-600`} />
                    </div>
                    <div>
                      <h3 className="text-lg font-bold text-gray-900">Area #{pred.id}</h3>
                ...
                {risk.level === 'High' && (
                  <div className="mt-4 bg-red-50 border border-red-200 rounded-lg p-3">
                    <div className="flex items-start gap-2">
                      ...
                      <div>
                        <p className="text-sm font-semibold text-red-900">High Risk Alert</p>
